...
| The definition | `definitions`, plaintext (it is the customer's own document) | the project's owner and its agent token | ≤ 1 MiB before anything parses it (`definitions/definition-size.ts`); re-validated on every read (#047, `runtime.service.ts`) |
